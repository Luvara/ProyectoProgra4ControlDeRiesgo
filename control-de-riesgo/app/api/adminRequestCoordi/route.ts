...
async function updateUserPermission(userId: number, permissons: string, department_dep_id: number) {
  try {
    const user = await prisma.user.update({
      where: { usu_id: userId },
      data: { 
        usu_permissons: permissons,
...
export async function PUT(req: NextRequest) {
  const { userId, permissons, department_dep_id } = await req.json();

  if (!userId || permissons === undefined || department_dep_id === undefined) {
    return NextResponse.json({ error: 'Invalid parameters' }, { status: 400 });
  }

  return updateUserPermission(userId, permissons, department_dep_id);
}
